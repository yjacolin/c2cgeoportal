...
The database migration repository is in ``<package>/CONST_migration`` but it 
must be modified int the ``update`` paste template.
...
``<package>/CONST_migration/versions/xxx_Explicite_name.py``
